    ...
    public partial class StaffDashboardForm : Form
    {
        ...
        private string GenerateISBN()
        {
            try
            {
                using (var connection = Helper.MYSqlHelper.CreateConnection())
                {
                        ...
                        LIMIT 1", connection))
                    {
                        ...
                        {
                            nextNumber = 1; // Reset or use timestamp-based approach
                            System.Diagnostics.Debug.WriteLine("ISBN sequence reset - reached maximum");
                        }
                        ...
                        // Verify it's unique
                        int attempts = 0;
                        while (IsISBNDuplicate(formattedISBN) && attempts < 100)
                        {
                            nextNumber++;
            ...
            catch (Exception ex)
            {
                ...
                string baseISBN = prefix + registrant + publication;
                int checkDigit = CalculateISBN13CheckDigit(baseISBN);
                return $"978-0-{registrant}-{publication}-{checkDigit}";
            }
        }
        ...
        private void ShowCatalogView()
        {
            ...
            // Search button (if exists) or Enter key
        ...
        private void LoadBooksData(string searchTerm = null, string category = null)
        {
            try
            {
                ...
                if (pnlBooksContainer == null) return;
                    ...
                    if (int.TryParse(txtCopies.Text, out int copies) && copies > 0)
                    {
                        try
                        {
                            ...
                            if (bookService.AddCopies(bookId, copies))
                            {
                                ...
                                // Refresh book list if we're in Catalog view
        ...
        private void ShowAddBookDialog()
        {
            ...
            btnAdd.Click += (s, e) => {
                try
                {
                    ...
                    {
                        DialogResult duplicateResult = MessageBox.Show(
                        ...
                        if (duplicateResult == DialogResult.No)
                        {
                            txtISBN.Focus();
                            return;
                        }
                    ...
                    if (bookId > 0)
                    {
                        ...
                        // Refresh book list if Catalog view is active
                        if (pnlMainContent.Tag?.ToString() == "CatalogView")
                        {
                            LoadBooksData();
                        }

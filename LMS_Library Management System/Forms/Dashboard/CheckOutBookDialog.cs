...
namespace LMS_Library_Management_System.Forms.Dashboard
{
    public class CheckOutBookDialog : Form
    {
        ...
        {
            // Update form properties for new design
            this.BackColor = Color.Beige;
            ...
            Button btnClose = new Button
            {
                ...
                Size = new Size(32, 32),
                Location = new Point(headerPanel.Width - 40, 12),
            ...
            Panel contentPanel = new Panel
            {
                Dock = DockStyle.Fill,
                BackColor = Color.Beige,
                Padding = new Padding(24, 24, 24, 24), // Increased top padding to prevent overlap
                AutoScroll = true
            };

            int yPos = 100; // Add top spacing for Select Member label
            int inputWidth = 400; // Reduced width for centering
            int contentWidth = contentPanel.Width - 48; // Available width minus padding
            int startX = (contentWidth - inputWidth) / 2; // Center the inputs
            ...
            yPos += 30; // Reduced spacing from label to input (30px for closer spacing)
            ...
            pnlMemberContainer.Paint += (s, e) =>
            {
                ...
                // Maroon border when focused, light gray when not
                Color borderColor = isFocused ? Color.FromArgb(128, 0, 32) : Color.FromArgb(200, 200, 200);
                int borderWidth = isFocused ? 2 : 1;
                using (GraphicsPath path = CreateRoundedRectangle(new Rectangle(0, 0, pnlMemberContainer.Width - 1, pnlMemberContainer.Height - 1), 8))
            ...
            {
                Location = new Point(14, 12), // Standard position
            ...
            Label lblMemberDropdown = new Label
            {
                ...
                ForeColor = Color.FromArgb(150, 150, 150),
                Location = new Point(inputWidth - 35, 13), // Standard position
                Size = new Size(25, 18),
            ...
            {
                Location = new Point(startX, yPos + 48),
            ...
            pnlMemberDropdown.Paint += (s, e) =>
            {
                ...
                // Draw subtle shadow
                using (GraphicsPath shadowPath = CreateRoundedRectangle(new Rectangle(2, 2, pnlMemberDropdown.Width - 3, pnlMemberDropdown.Height - 3), 8))
                {
                ...
                // Draw main border
                using (GraphicsPath path = CreateRoundedRectangle(new Rectangle(0, 0, pnlMemberDropdown.Width - 1, pnlMemberDropdown.Height - 1), 8))
                {
            ...
            pnlBookContainer.Paint += (s, e) =>
            {
                ...
                // Light gray border (not maroon like member field)
                Color borderColor = Color.FromArgb(200, 200, 200);
                int borderWidth = 1;
                using (GraphicsPath path = CreateRoundedRectangle(new Rectangle(0, 0, pnlBookContainer.Width - 1, pnlBookContainer.Height - 1), 8))
                using (Pen pen = new Pen(borderColor, borderWidth))
                {
                    e.Graphics.DrawPath(pen, path);
                }
            ...
            {
                Location = new Point(14, 18), // Added more top margin
                Size = new Size(inputWidth - 50, 20),
                Font = new Font("Segoe UI", 10F),
                BorderStyle = BorderStyle.None,
                BackColor = Color.White,
                ForeColor = Color.FromArgb(60, 60, 60),
                Margin = new Padding(0, 6, 0, 0) // Top margin
            };
            ...
            Label lblBookDropdown = new Label
            {
                ...
                Font = new Font("Segoe UI", 9F),
                ForeColor = Color.FromArgb(150, 150, 150),
                Location = new Point(inputWidth - 35, 19), // Adjusted for input margin
                Size = new Size(25, 18),
                TextAlign = ContentAlignment.MiddleCenter,
                Cursor = Cursors.Hand
            };
            ...
            pnlBookDropdown.Paint += (s, e) =>
            {
                ...
                // Draw subtle shadow
                using (GraphicsPath shadowPath = CreateRoundedRectangle(new Rectangle(2, 2, pnlBookDropdown.Width - 3, pnlBookDropdown.Height - 3), 8))
                {
                    using (SolidBrush shadowBrush = new SolidBrush(Color.FromArgb(30, 0, 0, 0)))
                    {
                        e.Graphics.FillPath(shadowBrush, shadowPath);
                ...
                // Draw main border
                using (GraphicsPath path = CreateRoundedRectangle(new Rectangle(0, 0, pnlBookDropdown.Width - 1, pnlBookDropdown.Height - 1), 8))
                {
                    e.Graphics.FillPath(Brushes.White, path);
                    using (Pen pen = new Pen(Color.FromArgb(200, 200, 200), 1))
                    {
            ...
            Button btnCancel = new Button
            {
                ...
                Location = new Point(inputWidth - 220, 5),
                Cursor = Cursors.Hand
            };
            ...
            Button btnIssue = new Button
            {
                ...
                BackColor = Color.FromArgb(128, 0, 32), // Maroon color
                FlatStyle = FlatStyle.Flat,
                FlatAppearance = { BorderSize = 0 },
                Size = new Size(120, 40),
                Location = new Point(inputWidth - 120, 5),
                Cursor = Cursors.Hand
            };
            ...
            // Update input positioning on resize (after all controls are created)
            contentPanel.Resize += (s, e) =>
            {
                int newContentWidth = contentPanel.Width - 48;
                int newStartX = (newContentWidth - inputWidth) / 2;
                
                if (lblMember != null)
                {
                    lblMember.Location = new Point(newStartX, lblMember.Location.Y);
                }
                if (pnlMemberContainer != null)
                {
                    pnlMemberContainer.Location = new Point(newStartX, pnlMemberContainer.Location.Y);
                }
                if (pnlMemberDropdown != null)
                {
                    pnlMemberDropdown.Location = new Point(newStartX, pnlMemberDropdown.Location.Y);
                }
                if (lblBook != null)
                {
                    lblBook.Location = new Point(newStartX, lblBook.Location.Y);
                }
                if (pnlBookContainer != null)
                {
                    pnlBookContainer.Location = new Point(newStartX, pnlBookContainer.Location.Y);
                }
                if (pnlBookDropdown != null)
                {
                    pnlBookDropdown.Location = new Point(newStartX, pnlBookDropdown.Location.Y);
                }
                if (buttonsPanel != null)
                {
                    buttonsPanel.Location = new Point(newStartX, buttonsPanel.Location.Y);
                    if (btnCancel != null) btnCancel.Location = new Point(inputWidth - 220, 5);
                    if (btnIssue != null) btnIssue.Location = new Point(inputWidth - 120, 5);
                }
            };
            ...
            {
                e.Graphics.SmoothingMode = SmoothingMode.AntiAlias;
                using (GraphicsPath path = CreateRoundedRectangle(new Rectangle(0, 0, this.Width - 1, this.Height - 1), 12))
                {
            ...
            // Focus on member search (but don't show dropdown)
            this.Shown += (s, e) => 
            {
        ...
        private void ShowMemberDropdown()
        {
            ...
            {
                // Limit to 3 items: 3 * 22 (item height) + 10 (padding) = 76
                int dropdownHeight = Math.Min(76, lstMembers.Items.Count * 22 + 10);
                pnlMemberDropdown.Height = dropdownHeight;
                pnlMemberDropdown.Location = new Point(pnlMemberContainer.Left, pnlMemberContainer.Bottom + 4);
        ...
        private void ShowBookDropdown()
        {
            ...
            {
                // Limit to 3 items: 3 * 22 (item height) + 10 (padding) = 76
                int dropdownHeight = Math.Min(76, lstBooks.Items.Count * 22 + 10);
                pnlBookDropdown.Height = dropdownHeight;
                pnlBookDropdown.Location = new Point(pnlBookContainer.Left, pnlBookContainer.Bottom + 4);
